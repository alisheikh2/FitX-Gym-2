...
export default function Footer() {
  return (
    <footer className="bg-deep border-t border-steel/50">
      <div className="shell py-16 grid gap-12 md:grid-cols-[1.3fr_1fr_1fr]">
        <div>
          <Logo />
          <p className="mt-5 text-sm text-silver leading-relaxed max-w-xs">{BRAND.tagline}</p>
        ...
        <nav aria-label="Footer explore">
          <h3 className="text-xs font-semibold uppercase tracking-[0.22em] text-muted mb-5">Explore</h3>
          <ul className="space-y-2.5 text-sm">
            {[['About', '/about'], ['Personal Training', '/personal-training'], ['Weight Loss', '/weight-loss'], ['Strength & Conditioning', '/strength-conditioning'], ["Women's Fitness", '/womens-fitness'], ['Trainers', '/trainers'], ['Results', '/results'], ['Facilities', '/facilities'], ['Body Assessment', '/body-assessment'], ['Resources', '/blog']].map(([l, to]) => (
              <li key={to}><Link to={to} className="text-silver hover:text-brand transition-colors">{l}</Link></li>
            ))}
          </ul>
        </nav>
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-[0.22em] text-muted mb-5">Studio</h3>
          <address className="not-italic text-sm text-silver leading-relaxed">{BRAND.address}</address>
          <p className="mt-3 text-sm text-silver">{BRAND.hoursWeek}<br />{BRAND.hoursFriday}</p>
          <p className="mt-3 text-sm"><a href={tel} className="text-paper font-semibold hover:text-brand">{BRAND.phoneDisplay}</a></p>
      ...
      <div className="border-t border-steel/50">
        <div className="shell py-5 flex flex-col sm:flex-row gap-3 items-center justify-between text-xs text-muted">
          ...
          <p className="flex gap-5">
            <Link to="/privacy-policy" className="hover:text-silver">Privacy</Link>
            <Link to="/terms-and-conditions" className="hover:text-silver">Terms</Link>
            <Link to="/admin" className="hover:text-silver">Staff</Link>
          </p>
        </div>
      </div>
    </footer>
